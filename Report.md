...
The solution is aimed primarily at **organisations that distribute images they hold the original of and need to trace where a leaked copy came from**: a film studio shipping a different copy to each cinema, a stock agency or press wire issuing a per-licensee copy, a content platform seeding a per-partner copy. When such a copy resurfaces where it should not be, the owner needs to identify *which* recipient is responsible; but by then the image has usually been re-compressed, resized and blurred by the re-upload path, and the obvious defences fail: a visible watermark defaces the asset (and is cropped out anyway), and metadata tags are stripped the moment the file is re-saved. Because Invisign embeds the mark in the **singular values of small image blocks** rather than in the raw pixels, it is invisible to the eye (~50 dB) yet survives exactly those lossy edits, which is what makes it useful here; and since the owner holds the master, the original-image requirement of extraction costs them nothing.

More broadly the method is relevant to copyright and authenticity use cases for any image owner (photographers, press agencies, stock providers). One boundary matters, though: it is a strong fit for *owner-held forensic tracing* and a weak one for settling *contested ownership* between two parties, for the reason developed in **Threat model & use cases**.
...
**Value proposition.** For an organisation that distributes images it holds the original of (a studio shipping a different copy to each cinema, a stock agency or press wire issuing a per-licensee copy, a platform seeding a per-partner copy), Invisign embeds a *different* invisible mark in each recipient's copy, so that when a copy leaks the owner can identify the responsible recipient by testing the leak against the known per-recipient marks (closed-set arg-max of the recovery lift). Its edge over what these owners use today is surviving the channel a leaked image actually passes through: a **visible** watermark defaces the asset and is cropped out, **EXIF/metadata** tags are stripped the moment the file is re-saved, and **simple LSB-style** invisible tools are wiped by re-compression and blur, whereas the block-singular-value mark holds up under the noise, JPEG re-encoding and blur of a web re-upload (see *Experiments*), at ~50 dB with no visible defacement. Non-blind extraction is no obstacle here, since the owner holds the master; the honest scope is owner-held forensic tracing, not contested ownership proof (see *Threat model & use cases*).
...
**Verdict.** The SVD core is a legitimate, working watermark at an imperceptible strength (≈ 50 dB), with genuine robustness to geometry, noise, JPEG and blur, and its honest home is **owner-held forensic tracing**, not contested ownership proof. That said, the additive-baseline comparison shows its margin is narrow: a one-line additive overlay matches or beats it on everything *except* the lossy/diffusive attacks (noise, JPEG, blur), so the marginal value of the SVD representation is specifically **robustness to lossy processing**. For how this positioning sits against commercial offerings (IMATAG, Digimarc, Verimatrix/NexGuard) and the adjacent AI-provenance wave, see **[Market.md](Market.md)**.
